...
export default function HouseholdForm() {
  ...
  const toggleDietaryPreference = (index: number, preference: DietaryPreference) => {
    setMembers((prevMembers) =>
      prevMembers.map((member, memberIndex) => {
        ...
        const exists = member.dietaryPreferences.includes(preference);

        return {
          ...member,
          dietaryPreferences: exists
            ? member.dietaryPreferences.filter((item) => item !== preference)
            : [...member.dietaryPreferences, preference],
        };
      })
    );
  };
  ...
  const addMember = () => {
    setMembers((prevMembers) => [...prevMembers, { ...initialMember }]);
  };
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      setError(
        submitError instanceof Error ? submitError.message : 'Failed to submit household health data'
      );
    } finally {
      setLoading(false);
    }
  ...
  return (
    <section className="household-health-section">
      <div className="household-health-header">
        ...
        <p>
          Add each household member and submit the profile.
        </p>
      </div>
          ...
          <article key={`member-${index}`} className="household-member-card">
            ...
            <div className="household-grid">
              ...
              <label className="household-field">
                ...
                <select
                  value={member.gender}
                  onChange={(event) => updateMember(index, 'gender', event.target.value as Gender)}
                >
              ...
              <label className="household-field">
                ...
                <input
                  ...
                  onChange={(event) => updateMember(index, 'weightRange', event.target.value)}
                  placeholder="60-70"
                />
              </label>
              ...
              <label className="household-field">
                ...
                <select
                  ...
                  onChange={(event) =>
                    updateMember(index, 'activityLevel', event.target.value as ActivityLevel)
                  }
                >
            ...
            <div className="household-preferences">
              <span className="household-preferences-label">Dietary preferences</span>
      ...
      <div className="household-actions">
        <button type="button" className="secondary-button" onClick={addMember} disabled={loading}>
          Add Member
        </button>
      ...
      {result ? (
        <div className="result-panel">
          <div className="result-header">
            <h3>NLP response</h3>
            {result.version ? <span className="result-version">{result.version}</span> : null}
          </div>

...
// GET /api/swadishtt/cart — the authenticated user's cart items + summary
export async function GET(request) {
  try {
    ...
    console.error('Error fetching Swadishtt cart:', error);
    return NextResponse.json({ error: 'Internal Server Error' }, { status: 500 });
  }
}

// DELETE /api/swadishtt/cart — clear the entire cart
export async function DELETE(request) {
  try {
    ...
    console.error('Error clearing Swadishtt cart:', error);
    return NextResponse.json({ error: 'Internal Server Error' }, { status: 500 });
  }
}
